...
function reportHtml(records: Checkup[]) {
  const rows = records.map((item) => {
    ...
    return `<tr><td>${esc(when)}</td><td>${esc(title)}</td><td>${item.intensity}/10${item.afterIntensity !== undefined ? ` (بعدها ${item.afterIntensity}/10)` : ''}</td><td>${esc(item.triageStatus ?? (item.urgent ? 'urgent' : 'routine'))}</td><td>${esc(item.redFlags?.join('، ') || '—')}</td><td>${esc(item.painType)}</td><td>${esc(item.duration)}</td><td>${esc(item.medication || '—')}</td><td>${esc(item.triggers || '—')}</td><td>${esc(item.note || '—')}</td></tr>`;
  }).join('');
